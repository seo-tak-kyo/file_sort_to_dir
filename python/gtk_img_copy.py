...
class Window(Gtk.ApplicationWindow):
    ...
    def on_start_copy_clicked(self, widget):
        ...
        copy_info["text_in_path"] = self.text_in_path.get_text()
        copy_info["text_out_path"] = self.text_out_path.get_text()
    ...
    def on_thread_working(self, copy_info):
        in_file_list = get_dir_list(copy_info["text_in_path"])
        ...
        for i in range(all_count):
            ...
            #tmp_path = self.text_out_path.get_text() + "/" + n_date.strftime("%Y/%m/%d/")
            # 2020/2020년8월
            tmp_path = copy_info["text_out_path"] + "/" + n_date.strftime("%Y/%Y년%m월/")
            ...
            #print("copy => " + n_file)
            file_copy(copy_info["text_in_path"] + "/" + n_file, tmp_path + n_file)
...
class MyWindow(Window):
    def __init__(self, title, width, height, **kwargs):
        ...
        lb_in_path = Gtk.Entry()
        lb_out_path = Gtk.Entry()
        lb_in_path.set_text(self.in_path)
        lb_out_path.set_text(self.out_path)
        lb_in_path.set_halign(Gtk.Align.START)
        lb_out_path.set_halign(Gtk.Align.START)
        content2_l.append(lb_in_path)
        content2_r.append(lb_out_path)
        ...
        lb_result = Gtk.Label()
        lb_result.set_text('copy : 0/0')
        content.append(lb_result)
